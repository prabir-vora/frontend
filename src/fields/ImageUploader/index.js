...
class ImageUploader extends Component {
    ...
    // Call Action Creator to upload file to Amazon S3

    uploadFile = async (file, typeOfUpload) => {
        const { actionCreators } = AdminDuck;
        const { uploadImage } = actionCreators;
        console.log(file);
        console.log(typeOfUpload);
        return await this.props.dispatch(uploadImage(file, typeOfUpload));
    ...
    onSubmit = async (e) => {
        ...
        const { success, imageURL, message } = await this.props.dispatch(uploadImage(file, typeOfUpload));
        if (success) {
            ShowConfirmNotif({
                message,
                type: "success"
    ...
    renderUploadForm= () => (
        <form style={{ width: '100%', paddingBottom: '10px', textAlign: 'center' }} onSubmit={this.onSubmit}>
            <input
